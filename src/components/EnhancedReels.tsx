...
const reelsData = [
    {
        id: "1",
        content:
            "Operating Systems manage computer resources, acting as an interface between users and hardware. They handle process scheduling, memory allocation, and provide an efficient environment for program execution.",
    },
    {
        id: "2",
        content:
            "Process states include New, Ready, Run, Waiting, and Terminated. OS manages these states using Job, Ready, and Waiting queues, optimizing system performance and resource utilization.",
    },
    {
        id: "3",
        content:
            "Virtual Memory allows executing processes not completely in memory. It creates an illusion of larger memory by using secondary storage, enabling more efficient multi-programming.",
    },
    {
        id: "4",
        content:
            "Deadlock occurs when processes are stuck waiting for resources in a circular dependency. Four conditions must exist: Mutual Exclusion, Hold and Wait, No Preemption, and Circular Wait.",
    },
    {
        id: "5",
        content:
            "Threads are lightweight processes within a single process. They enable concurrent execution of tasks, sharing the same memory space and allowing more responsive and efficient program design.",
    },
    {
        id: "6",
        content:
            "Page Replacement Algorithms manage memory by deciding which pages to remove when memory is full. Strategies include FIFO, Least Recently Used (LRU), and Optimal algorithms.",
    },
    {
        id: "7",
        content:
            "System Calls are the interface between user programs and the kernel. They allow processes to request services from the operating system, such as file management, process control, and device operations.",
    },
    {
        id: "8",
        content:
            "Paging divides physical memory into fixed-size blocks called frames and logical memory into pages. The Page Table maps these, enabling non-contiguous memory allocation.",
    },
    {
        id: "9",
        content:
            "Concurrency involves multiple instruction sequences executing simultaneously. It requires careful synchronization to prevent race conditions and ensure data consistency.",
    },
    {
        id: "10",
        content:
            "The Memory Management Unit (MMU) translates virtual addresses to physical addresses, providing memory protection and enabling efficient memory usage across different processes.",
    },
];
...
export default function EnhancedReels() {
        ...
        const handleScroll = () => {
            if (containerRef.current) {
                ...
                if (index !== currentIndex && index >= 0 && index < reelsData.length) {
                    ...
                    if (autoSpeak) {
                        speakText(reelsData[index].content);
                    }
                }
            }
        };
    ...
    const toggleAutoSpeak = () => {
        ...
        if (newState) {
            speakText(reelsData[currentIndex].content);
        } else {
                        ...
                        <div className={styles.contentWrapper}>
                            ...
                            <div className={styles.content}>
                                <div className={styles.textContainer}>
                                    <p className={styles.text}>{reel.content}</p>
                                </div>
                            </div>
                        </div>
                    </div>
